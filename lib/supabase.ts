...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    autoRefreshToken: false, // We manage tokens ourselves
    persistSession: false, // We use our own JWT system
  },
});
...
/**
 * Set your custom JWT in Supabase client
 * Call this after user login/refresh to enable auth.uid() in RLS policies
 * @param accessToken - Your custom JWT access token
 */
export function setSupabaseAuth(accessToken: string) {
  // This makes Supabase include your JWT in all requests
  // and makes auth.uid() work in RLS policies
  supabase.auth.setSession({
    access_token: accessToken,
    refresh_token: "", // Not used since we manage refresh ourselves
  });
  console.log("[Supabase] Auth session set with JWT");
}
...
export interface WhatsAppMessage {
  ...
  user_id: string; // TEXT type to match whatsapp_accounts.user_id
  from_number: string;
