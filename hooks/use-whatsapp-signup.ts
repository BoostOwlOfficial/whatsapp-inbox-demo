...
export function useWhatsAppSignup() {
    ...
    const initializeSignup = useCallback(async () => {
        try {
            ...
            const errorMessage = err instanceof Error ? err.message : 'Failed to initialize signup'
    ...
    const launchSignup = useCallback(async () => {
        try {
                        ...
                        if (response.authResponse) {
                            ...
                            const callbackResponse = await fetch('/api/whatsapp/signup/callback', {
                                ...
                                body: JSON.stringify({
                                    ...
                                    wabaId: response.authResponse.granted_scopes?.includes('whatsapp_business_management')
            ...
            const errorMessage = err instanceof Error ? err.message : 'Failed to launch signup'
            setError(errorMessage)
            setIsLoading(false)
            throw err
        }
