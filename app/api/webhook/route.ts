            ...
            for (const message of messages) {
              ...
              try {
                ...
                if (
                  ...
                  console.warn(
                    "Supabase not configured (missing URL or Service Role Key), skipping message storage"
                  );
                  continue;
                }

                // Import retry utility
                const { retrySupabaseOperation } = await import(
                  "@/lib/retry-utils"
                );

                // SECURITY: Lookup user_id using admin client (aliased as supabase)
                // This links the message to the correct user for RLS filtering
                let account: { user_id: string } | null = null;
                ...
                await retrySupabaseOperation(async () => {
                  ...
                  if (result.error && result.error.code !== "PGRST116") {
                    // Throw specific errors to trigger retry, but not "Data not found" (PGRST116)
                    throw result.error;
                  }
                }, "Lookup user account");
                ...
                if (accountError || !account?.user_id) {
                  ...
                  continue; // Skip this message if no user found
                }
                ...
                const messageData = {
                  ...
                  user_id: account.user_id, // CRITICAL: Links message to user for RLS
                ...
                // Retry the insert operation using admin client
                await retrySupabaseOperation(async () => {
            ...
            for (const status of statuses) {
              ...
              try {
                ...
                if (
                  ...
                  console.warn(
                    "Supabase not configured (missing URL or Service Role Key), skipping status update"
                  );
                  continue;
                }
                ...
                // Retry the update operation using admin client
                await retrySupabaseOperation(async () => {
                  const { error } = await supabase
                    .from("whatsapp_messages")
