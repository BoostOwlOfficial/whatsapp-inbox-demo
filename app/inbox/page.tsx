...
export default function InboxPage() {
  ...
  const [activeTab, setActiveTab] = useState<"open" | "unread" | "my_chats">(
    "open",
  );
      ...
      if (accessToken) {
        try {
          ...
          if (response.ok) {
            ...
            setAiAutoReplyEnabled(data.use_ai_to_reply || false);
          }
        } catch (error) {
  ...
  const handleStartNewChat = () => {
    ...
    const existingConv = conversations.find(
      (c) => c.recipientPhone === formattedPhone,
    );
    if (existingConv) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success && result.message_id) {
        ...
        console.log(
          `✅ Message sent successfully. Updating ID: ${tempId} → ${result.message_id}`,
        );
        ...
        <div className="flex h-full items-center justify-center bg-slate-50">
          <div className="text-center max-w-md p-8">
            ...
            <p className="text-slate-600 mb-6">
              Please connect your WhatsApp Business account in Settings to start
              using the Inbox.
            </p>
            <Button
    ...
    <ProtectedRoute>
      <div className="flex h-full bg-white overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="divide-y divide-slate-50">
                {filteredConversations.map((conv) => {
                  ...
                  ][contactName.length % 4 || 0];

                  return (
                    <div
                      key={conv.id}
                      onClick={() => setSelectedConversation(conv)}
                      className={`relative p-4 cursor-pointer hover:bg-slate-50 transition-colors ${
                        selectedConversation?.id === conv.id
                          ? "bg-slate-50 border-l-4 border-green-500"
                          : "border-l-4 border-transparent"
                      }`}
                    >
                      <div className="flex gap-3">
                        <Avatar
                          className={`h-10 w-10 border border-slate-100 ${avatarColor}`}
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {conv.tags.slice(0, 2).map((tag) => (
                              <span
                                key={tag}
                                className="px-1.5 py-0.5 rounded text-[10px] bg-slate-100 text-slate-600 border border-slate-200 font-medium"
                              >
                                {tag}
                              </span>
                            ))}
        ...
        {selectedConversation ? (
          <div
            className="flex-1 flex flex-col bg-[#efeae2] overflow-hidden"
            style={{
              backgroundImage:
            ...
            <div className="flex-1 overflow-y-auto p-6 min-h-0 scroll-smooth">
              <div className="space-y-6">
                ...
                {selectedConversation.messages.map((msg) => {
                  ...
                  const isAIResponse =
                    isOutbound &&
                    msg.metadata?.sent_via === "whatsapp_send_utility";
                  return (
                    <div
                      key={msg.id}
                      className={`flex ${
                    ...
                    >
                      <div
                        className={`max-w-[60%] rounded-lg p-3 shadow-sm relative text-sm ${
                          isOutbound
                      ...
                      >
                        {isAIResponse && (
                          <div className="flex items-center gap-1 text-[10px] text-green-700 mb-1">
                        ...
                        <div
                          className={`text-[10px] mt-1 flex items-center justify-end gap-1 ${
                            isOutbound ? "text-green-700/70" : "text-slate-400"
                          }`}
                        >
